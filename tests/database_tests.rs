...
async fn test_task_state_operations() {
    ...
    // Add state
    db.add_valid_state(&powershell_task, &powershell_state).unwrap();
    ...
    // Delete state
    db.delete_valid_state(&powershell_task, &powershell_state).unwrap();
...
async fn test_config_merge() -> Result<()> {
    ...
    config.task_states.gitversion.push(GitVersionState::new("3", "3", "6.0.3"));
    ...
    let powershell_states = db.list_valid_states(&SupportedTask::Default("powershell".to_string()))?;
    assert_eq!(powershell_states.len(), 2);
    assert!(powershell_states.iter().any(|s| matches!(s, TaskValidState::Default(v) if v == "2.0")));
    assert!(powershell_states.iter().any(|s| matches!(s, TaskValidState::Default(v) if v == "3.0")));
